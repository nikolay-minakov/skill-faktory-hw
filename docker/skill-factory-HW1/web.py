...
def get_db_time():
        # Тут я  подредактировал строку connect убрав format. Не модно и не молодежно. Вместо него добавил f строку. 
        conn = connect(f"host= {get_db_config('db_host')} dbname={get_db_config('db_name')} user={get_db_config('db_user')} password={get_db_config('db_password')} port= {get_db_config('db_port')}")
